// SAL Anomaly Dashboard — Station comparison (Leaflet map + Plotly time series)
(function () {
  'use strict';

  // -----------------------------------------------------------------------
  // Run configuration — add more runs here without touching anything else.
  // Each entry: { key: JSON key prefix, label: display name, color: trace color }
  // The first run is used for map marker coloring.
  // -----------------------------------------------------------------------
  ...
  // Per-run visibility state (all on by default)
  var runVisible = {};
  ...
  // Scale integer mm values to metres. null passthrough.
  function mmToM(arr) {
  ...
  function buildTracesForRange(data, startIdx, endIdx, target) {
    ...
    // Use obs as primary series for LTTB; fall back to first visible run
    var primaryKey = 'obs';
    var primaryArr = data[primaryKey] ? data[primaryKey].slice(startIdx, endIdx) : null;
    if (!primaryArr) {
    ...
    // Build time axis for selected indices
  ...
  function renderTimeseries(data, plotDiv) {
    ...
    plotDiv.on('plotly_relayout', function (evt) {
      ...
      Plotly.react(plotDiv, refined, plotDiv.layout, {
        responsive: true,
        displaylogo: false,
        modeBarButtonsToRemove: ['lasso2d', 'select2d'],
      });
