// SAL Anomaly Dashboard — SAL-anomaly-tide-only run (Oct–Dec 2017)
(function () {
  'use strict';

  var RUNS = [
    { key: 'sal-tide-only', label: 'SAL Tide Only', color: '#e377c2' },
  ];
  var OBS_COLOR = '#000000';

  var DATA_PREFIX = 'data/sal-tide-only/';
  ...
  // -----------------------------------------------------------------------
  // Marker coloring (by SSH RMSE vs GESLA obs)
  // -----------------------------------------------------------------------
  ...
  fetch(DATA_PREFIX + 'stations.json')
    ...
    .then(function (stations) {
      ...
      stations.forEach(function (stn) {
        ...
        if (stn.metrics && stn.metrics[primaryKey]) {
          var m = stn.metrics[primaryKey];
  ...
  function buildTracesForRange(data, startIdx, endIdx, target) {
    ...
    var primaryArr = data['obs']
      ? data['obs'].slice(startIdx, endIdx)
      : (data[RUNS[0].key] ? data[RUNS[0].key].slice(startIdx, endIdx) : null);
